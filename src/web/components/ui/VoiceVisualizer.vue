...
function shapeAt(angle: number, time: number): number {
  return (
    ...
    (0.5 + 0.5 * Math.cos(angle * 7 - time * 1.9)) * 0.3 +
  ...
  setup(props) {
    ...
    const lines = computed(() => {
      ...
      return Array.from({ length: LINE_COUNT }, (_, index) => {
        ...
        return {
          ...
          x2: CENTER + cos * end,
          y2: CENTER + sin * end,
          opacity: 0.2 + amplitude * (0.24 + shape * 0.56),
        };
      });
    });
    ...
    const waves = computed(() => {
      ...
      return Array.from({ length: 4 }, (_, layer) => ({
        ...
        opacity:
          props.variant === "waves"
            ? (0.2 + amplitude * 0.56) * (1 - layer * 0.22)
            : (0.1 + amplitude * 0.28) * (1 - layer * 0.22),
      }));
    });
  ...
  >
    <svg class="voice-visualizer__canvas" viewBox="0 0 320 320" fill="none" aria-hidden="true">
      ...
      <circle
        cx="160"
        cy="160"
        :r="54 + frame.amplitude * 2"
        fill="currentColor"
        :fill-opacity="0.022 + frame.amplitude * 0.016"
      />
      <circle
        cx="160"
        cy="160"
      ...
      <slot :value="frame.amplitude">
        <svg viewBox="0 0 40 40" class="voice-visualizer__glyph" fill="none" aria-hidden="true">
          <g
            ...
            stroke-width="2.7"
            stroke-linecap="round"
            :opacity="0.45 + frame.amplitude * 0.5"
          >
            <line
              v-for="(bar, index) in centerBars"
              :key="index"
              :x1="bar.x"
              :x2="bar.x"
              :y1="bar.y1"
