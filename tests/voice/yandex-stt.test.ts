...
test("final fragments emitted before commit are retained for that utterance", async () => {
  const stream = new FakeStream();
  const events: YandexSttEvent[] = [];
  const stt = createYandexStt({
    streamFactory: async () => stream,
  ...
  stream.respond({
    session_uuid: { uuid: "s" },
    audio_cursors: { final_index: 1 },
    final: { alternatives: [{ text: "early" }] },
  });
  assert.equal(events.filter((event) => event.type === "final").length, 0);
  stt.commit("turn-1");
  ...
  const finals = events.filter((event) => event.type === "final");
  assert.equal(finals.length, 1);
  assert.equal(finals[0]?.utteranceId, "turn-1");
  assert.equal(finals[0]?.text, "early");
  stream.respond({
    final_refinement: {
      final_index: 1,
      normalized_text: { alternatives: [{ text: "refined" }] },
    },
  });
  const refinements = events.filter((event) => event.type === "refinement");
  assert.equal(refinements[0]?.utteranceId, "turn-1");
  assert.equal(refinements[0]?.text, "refined");
  stt.disconnect();
});

test("Yandex canStartUtterance reflects a clean stream boundary", async () => {
  const stream = new FakeStream();
  const stt = createYandexStt({
    streamFactory: async () => stream,
    onEvent: () => {},
  ...
  assert.equal(stt.canStartUtterance, false);
  await stt.connect();
  assert.equal(stt.canStartUtterance, true);
  ...
  stream.respond({ eou_update: { time_ms: 200 } });
  assert.equal(stt.canStartUtterance, false);
  assert.throws(() => stt.commit("turn-3"), /previous utterance/);
  assert.equal(stt.canStartUtterance, true);
...
test("new audio does not enter the stream while an earlier EOU is pending", async () => {
  const stream = new FakeStream();
  const events: YandexSttEvent[] = [];
  const stt = createYandexStt({
    streamFactory: async () => stream,
  ...
  assert.equal(stt.sendAudioChunk(Buffer.from([1, 0])), true);
  stt.commit("turn-1");
  const writesBeforeNextTurn = stream.requests.length;
  assert.equal(stt.sendAudioChunk(Buffer.from([2, 0])), false);
  assert.equal(stream.requests.length, writesBeforeNextTurn);
  stream.respond({ final: { alternatives: [{ text: "old" }] } });
  stream.respond({ eou_update: { time_ms: 100 } });
  assert.equal(stt.sendAudioChunk(Buffer.from([3, 0])), false);
  assert.throws(() => stt.commit("turn-2"), /previous utterance/);
  assert.deepEqual(
    events.filter((event) => event.type === "final").map((event) => event.utteranceId),
    ["turn-1"],
  );
  assert.equal(stt.sendAudioChunk(Buffer.from([4, 0])), true);
  stream.respond({ final: { alternatives: [{ text: "new" }] } });
  stt.commit("turn-3");
  stream.respond({ eou_update: { time_ms: 200 } });
  assert.deepEqual(
    events.filter((event) => event.type === "final").map((event) => event.utteranceId),
